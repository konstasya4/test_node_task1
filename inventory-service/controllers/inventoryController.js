...
increaseStock: async (req, res) => {
  try {
    ...
    const result = await inventoryService.increaseStock(product_id, shop_id, quantity);
...
decreaseStock: async (req, res) => {
  try {
    const { product_id, shop_id, quantity } = req.body;
    const result = await inventoryService.decreaseStock(product_id, shop_id, quantity);
    res.status(200).json(result);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
