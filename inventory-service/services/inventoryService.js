...
async function findStocks(product_id, shop_id) {
  const stock = await db.findOne("inventory.product_stocks", {
    product_id,
    shop_id,
  });
  if (!stock) {
...
module.exports = {
  ...
  createStock: async (
    ...
    try {
      ...
      const product = await db.findOne("inventory.products", {
        id: product_id,
      });
  ...
  increaseStock: async (product_id, shop_id, quantity) => {
    try {
      ...
      const updatedStock = await db.update(
        "inventory.product_stocks",
        { product_id, shop_id },
        stock
      );
      await sendToHistoryService("INCREASE_STOCK", {
        plu: product.plu,
        shop_id,
      });
      return updatedStock;
  ...
  decreaseStock: async (product_id, shop_id, quantity) => {
    try {
      ...
      await sendToHistoryService("DECREASE_STOCK", {
        plu: product.plu,
        shop_id,
      });
      return updatedStock;
